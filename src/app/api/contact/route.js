...
const generateEmailContent = (data) => {
  ...
  const htmlData = Object.entries(data).reduce((str, [key, val]) => {
    return (str += `<h3 class="form-heading">${CONTACT_MESSAGE_FIELDS[key]}</h3><p class="form-answer">${val}</p>`);
  }, "");
...
export async function POST(request) {
  try {
    ...
    if (!data || !data.name || !data.email || !data.number || !data.service || !data.message) {
...
//       subject: `New message from ${res.name}`,
//     });
