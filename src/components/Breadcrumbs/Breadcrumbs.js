  ...
  return (
    <div className="bg-black relative overflow-hidden">
      <div className="py-16 mx-auto max-w-7xl p-6 lg:px-8 flex items-center justify-between">
        <h1 className="text-xl md:text-2xl font-extrabold text-white">{name}</h1>

        <div className="flex items-center text-sm md:text-base text-white">
          <Link href="/" className="breadcrumbs-btn text-white">Home</Link>
          <div className="h-4 w-4 text-white">
            <ChevronRightIcon />
          </div>
          <p className="breadcrumbs-btn text-white">{name}</p>
        </div>
      </div>
    </div>
  );
